...
import com.teamobi.mobiarmy2.army2.fight.boss.Ghost2;
...
public class Player {
    ...
    private final boolean[] itemclan = new boolean[ItemClanData.entrys.size() + 1];
    ...
    public Player(FightManager fightMNG, byte location, short X, short Y, int[] item, int teamPoint, User us) {
        ...
        if (this.us.getisNHTGAMEItem2(75)) {
            this.dongDoi += this.dongDoi * 50 / 100;
            this.HPMax += this.HPMax * 50 / 100;
            this.phongThu += this.phongThu * 50 / 100;
            this.mayMan += this.mayMan * 50 / 100;
        }
        this.HP = this.HPMax;
    }
    ...
    public void collision(short bx, short by, Bullet bull) {
        ...
        if (dame > 0) {
            ...
            if (bull.pl.us != null && bull.pl.us.getisNHTGAMEItem2(74)) {
                this.NHTItemThieuDot[bull.pl.index][0] = 3;
                this.NHTItemThieuDot[bull.pl.index][1] = bull.pl.us.getLevel();
            }
            if (bull.pl.us != null && bull.pl.us.getisNHTGAMEItem2(75)) {
                dame += dame * 50 / 100;
            }
...
//        dame = Math.round(dame - (dame * (PhongThu * 100 / maxPhongThu) / 100));
        ...
        if (dame > 0) {
            ...
            if (bull.pl.us.getisNHTGAMEItem2(76)) {
                bull.pl.updateHP((int) (dame * 1 / 100));
            }
                    ...
                    if (this.idNV == 23) {
                        switch (Until.nextInt(4)) {
                            ...
                            case 0:
                                ...
                                bull.pl.GiftBoxFalling.get(bull.pl.GiftBoxFalling.size() - 1).Coins(xuup[Until.nextInt(xuup.length)]);
                                break;
                            //item
                            case 1:
                                bull.pl.GiftBoxFalling.add(new box());
                                bull.pl.GiftBoxFalling.get(bull.pl.GiftBoxFalling.size() - 1).Item((byte) (Until.nextInt(ItemData.entrys.size() - 2) + 2), (byte) Until.nextInt(1, 5));
                                break;
                            //xp
                            case 2:
                                bull.pl.GiftBoxFalling.add(new box());
                                bull.pl.GiftBoxFalling.get(bull.pl.GiftBoxFalling.size() - 1).XP(Until.nextInt(20000, 30000));
                                break;
                            //item dac biet
                            case 3:
                                bull.pl.GiftBoxFalling.add(new box());
                                bull.pl.GiftBoxFalling.get(bull.pl.GiftBoxFalling.size() - 1).SpecialItem((byte) (Until.nextInt(8) + Until.nextInt(5) * 10), 1);
                                break;
                        }
                        ...
                        switch (Until.nextInt(4)) {
                            ...
                            case 0:
                                ...
                                bull.pl.GiftBox.get(bull.pl.GiftBox.size() - 1).Coins(xuup[Until.nextInt(xuup.length)]);
                                break;
                            //item
                            case 1:
                                bull.pl.GiftBox.add(new box());
                                bull.pl.GiftBox.get(bull.pl.GiftBox.size() - 1).Item((byte) (Until.nextInt(ItemData.entrys.size() - 2) + 2), (byte) Until.nextInt(1, 5));
                                break;
                            //xp
                            case 2:
                                bull.pl.GiftBox.add(new box());
                                bull.pl.GiftBox.get(bull.pl.GiftBox.size() - 1).XP(Until.nextInt(1, 127));
                                break;
                            //item dac biet
                            case 3:
                                bull.pl.GiftBox.add(new box());
                                bull.pl.GiftBox.get(bull.pl.GiftBox.size() - 1).SpecialItem((byte) (Until.nextInt(8) + Until.nextInt(5) * 10), 1);
                                break;
                        }
                    } else if (this.idNV == 26) {
                        for (int i = 0; i < 2; i++) {
                            Player players = new Ghost2(fightMNG, (byte) 26, "Ghost II", (byte) (fightMNG.allCount + fightMNG.bullMNG.addboss.size()), 1800 + (fightMNG.getLevelTeam() * 10), (short) (Until.nextInt(100, fightMNG.mapMNG.Width - 100)), (short) Until.nextInt(150));
                            fightMNG.bullMNG.addboss.add(new AddBoss(players, fightMNG.getisLH() ? 50 : 6));
                        }
                    } else if (fightMNG.getisSieuBoss()) {
                        for (int i = 0; i < 5; i++) {
                            Player players = new BigBoomHTCC(fightMNG, (byte) 12, "BigBoomHTCC", (byte) (fightMNG.allCount + fightMNG.bullMNG.addboss.size()), 1500 + (fightMNG.getLevelTeam() * 100), (short) (Until.nextInt(100, fightMNG.mapMNG.Width - 100)), (short) Until.nextInt(150));
                    ...
                    if (this instanceof Boss) {
                        if (fightMNG.mapMNG.Id != 35 && this.idNV != 23 && this.idNV != 24) {
                            switch (Until.nextInt(3)) {
                                case 0:
                                    int kichno = Until.nextInt(10, 20);
                                    bull.pl.updateAngry(kichno);
                                    bull.pl.flyNotice(kichno + " Siêu kích Power");
                                    break;
                                case 1:
                                    int xuroi = Until.nextInt(1, 100);
                                    bull.pl.us.updateXu(xuroi);
                                    bull.pl.flyNotice(xuroi + " Xu chiến đấu");
                                    break;
                                case 2:
                                    int cuplen = Until.nextInt(1, 20);
                                    bull.pl.updateCUP(cuplen);
                                    bull.pl.flyNotice(cuplen + " Danh dự chiến đấu");
                                    break;
                            }
                        }
                        if (fightMNG.mapMNG.Id != 35 && this.idNV != 23 && this.idNV != 24 && !fightMNG.getisSieuBoss()) {
                            switch (Until.nextInt(2)) {
                                case 0:
                                    int soluong = Until.nextInt(1, 3);
                                    bull.pl.us.updateSpecialItem(77, soluong);
                                    bull.pl.flyNotice(soluong + " Hoa tuyết Christmas");
                                    break;
                                case 1:
                                    int soluong1 = Until.nextInt(1, 3);
                                    bull.pl.us.updateSpecialItem(77, soluong1);
                                    bull.pl.flyNotice(soluong1 + " Hoa tuyết Christmas");
                                    break;
                            }
                        }
                        if (fightMNG.getisSieuBoss() && this.idNV != 23 && this.idNV != 24) {
                            switch (Until.nextInt(7)) {
                                case 0:
                                    bull.pl.us.updateLuong(1);
                                    bull.pl.flyNotice(1 + " Lượng Vùng Cấm Địa");
                                    break;
                                case 1:
                                    bull.pl.us.updateLuong(0);
                                    bull.pl.flyNotice(0 + " Lượng Vùng Cấm Địa");
                                    break;
                                case 2:
                                    bull.pl.us.updateLuong(0);
                                    bull.pl.flyNotice(0 + " Lượng Vùng Cấm Địa");
