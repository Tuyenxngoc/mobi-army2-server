...
public class FightWait {
    ...
    public void enterFireOval(User us) throws IOException {
        ...
        synchronized (this.players) {
            ...
            if (this.isSieuBoss) {
                ms = new Message(45);
                ds = ms.writer();
                ds.writeUTF("Chào mừng bạn đến với Vùng Cấm Địa, Hãy tiêu diệt quái vật cà dành thắng lợi !");
                ds.flush();
                us.sendMessage(ms);
            }
        }
    ...
    public void chatMessage(User us, Message ms) throws IOException {
        ...
        DataOutputStream ds;
        if (s.equalsIgnoreCase("diem danh") || s.equalsIgnoreCase("điểm danh") || s.equalsIgnoreCase("điểm danh hàng ngày") || s.equalsIgnoreCase("diem danh hang ngay")) {
            ms = new Message(45);
            ds = ms.writer();
            try {
                if (Until.compare_Day(new Date(), us.getBaodanhsk())) {
                    ...
                    ds.writeUTF("Điểm danh thành công tích cực điểm danh để nhận giftcode nhé");
                } else {
                    ds.writeUTF("Bạn đã điểm danh hôm nay rồi, hãy quay lại vào ngày mai nhé!");
                }
    ...
    public void startMessage(Session cl) throws IOException {
        Message ms;
        DataOutputStream ds;
...
//        //if (this.isSieuBoss && hours >= 6 && hours <= 10 || hours >= 13 && hours <= 15 || hours >= 19 && hours <= 24) {
//        if (this.isSieuBoss && hours >= 0 && hours <= 6 && hours >= 10 && hours <= 13 && hours >= 15 && hours <= 19) {
//            ms = new Message(45);
//            ds = ms.writer();
//            ds.writeUTF("Hiện tại Vùng Cấm Địa chưa đến giờ mở ! " + " Giờ hiện tại : " + hours + " : " + minute);
//            ds.flush();
//            cl.sendMessage(ms);
//            return;
//        }
        ...
        for (byte i = 0; i < this.players.length; i++) {
            ...
            for (byte j = 0; j < 8; j++) {
                ...
                //if (itemMap[i][itemSelect] <= ItemData.nItemDcMang[itemSelect] && itemMap[i][itemSelect] <= pl.getItemNum(itemSelect) && (j <= 4 || pl.getItemNum(12 + j - 4) != 0)) continue; {
                if ((itemMap[i][itemSelect] > ItemData.nItemDcMang[itemSelect]) && (itemMap[i][itemSelect] > pl.getItemNum(itemSelect)) && (j > 4 && pl.getItemNum(12 + j - 4) == 0)) {
                    ms = new Message(45);
                    ds = ms.writer();
    ...
    public void findPlayerMessage(User us, Message ms) throws IOException {
        ...
        if (find) {
            ...
            for (User us0 : uss) {
                ...
                ds.writeByte("2.2.3".equals(us.client.version) ? (lvS > 127 ? 127 : lvS) : (lvS > 255 ? 255 : lvS));
