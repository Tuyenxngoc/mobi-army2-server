package com.teamobi.mobiarmy2.army2.fight;

import com.teamobi.mobiarmy2.army2.fight.BulletManager.AddBoss;
import com.teamobi.mobiarmy2.army2.fight.BulletManager.BomHenGio;
import com.teamobi.mobiarmy2.army2.fight.BulletManager.Bullets;
import com.teamobi.mobiarmy2.army2.fight.BulletManager.VoiRong;
import com.teamobi.mobiarmy2.army2.fight.boss.*;
import com.teamobi.mobiarmy2.army2.fight.bullet.ItemBomHenGio;
import com.teamobi.mobiarmy2.army2.server.*;
...
public class FightManager {
    ...
    private void nextBoss() throws IOException {
        ...
        if (this.wait.map == 32) {
            ...
            for (byte i = 0; i < numBoss; i++) {
                players[allCount] = new SpiderMachine(this, (byte) 13, "Spider Robot", (byte) allCount, 4785 + (getLevelTeam() * 15), tempX[i], tempY[i]);
                allCount++;
            }
        }
        //map thanh pho may
        ...
        if (this.wait.map == 40) {
            short X = (short) (Until.nextInt(300, 800));
            short Y = (short) Until.nextInt(-350, 50);
            players[allCount] = new Balloon(this, (byte) 17, "Balloon", (byte) allCount, 1, X, Y);
            allCount++;
            players[allCount] = new Balloon_Gun(this, (byte) 18, "Balloon Gun", (byte) allCount, 2000 + (this.getLevelTeam() * 10), (short) (X + 51), (short) (Y + 19));
            allCount++;
            ...
            allCount++;
            for (byte i = 0; i < 10; i++) {
                X = (short) ((i % 2 == 0) ? Until.nextInt(75, 150) : Until.nextInt(1050, 1125));
                Y = (short) 200;
        ...
        if (this.wait.map == 42) {
            for (byte i = 0; i < 30; i++) {
                short X = (short) (Until.nextInt(300, 800));
                short Y = (short) Until.nextInt(0, 1);
                X = (short) ((i % 2 == 0) ? Until.nextInt(20, 530) : Until.nextInt(950, 1450));
                Y = (short) 500;
        ...
        if (this.wait.map == 43) {
            short X = (short) (Until.nextInt(275, 830));
            short Y = (short) Until.nextInt(95, 128);
            players[allCount] = new Balloon(this, (byte) 17, "Balloon", (byte) allCount, 1, X, Y);
            allCount++;
        ...
        if (this.wait.map == 44) {
            for (byte i = 0; i < 5; i++) {
                short X = (short) (Until.nextInt(361, 710));
                short Y = (short) (172);
...
//            for (byte i = 0; i < 1; i++) {
//                short x, y;
//                x = (short) Until.nextInt(20, this.mapMNG.Width - 20);
//                y = (short) Until.nextInt(100, 100);
...
//        }
    ...
    private void fightComplete(byte checkWin) throws IOException {
        ...
        if (!this.wait.isLH && !wait.isSieuBoss && this.type == 5 && checkWin == 1) {
            for (byte i = 0; i < ServerManager.maxPlayers; i++) {
                Player pl = this.players[i];
                if (pl == null || pl.us == null) {
                    continue;
                }
                if (this.mapMNG.Id == 40) {//sp
                    byte idSpItem = new byte[]{122, 123, 124}[Until.nextInt(3)];
                    int numSpItem = Until.nextInt(5, 10);
                    pl.us.updateSpecialItem(idSpItem, numSpItem);
                    pl.us.sendMSSToUser(null, "Thành công chinh phục Siêu Boss nhận được: " + SpecialItemData.getSpecialItemById(idSpItem).name + " x" + numSpItem);
                    ms = new Message(46);
                    ds = ms.writer();
                    ds.flush();
                    sendToServer(ms);
                }
                if (this.mapMNG.Id == 41) {//tg
                    byte idSpItem = new byte[]{122, 123, 124}[Until.nextInt(3)];
                    int numSpItem = Until.nextInt(10, 15);
                    pl.us.updateSpecialItem(idSpItem, numSpItem);
                    pl.us.sendMSSToUser(null, "Thành công chinh phục BOSS Thế Giới nhận được: " + SpecialItemData.getSpecialItemById(idSpItem).name + " x" + numSpItem);
                    ms = new Message(46);
                    ds = ms.writer();
                    ds.flush();
                    sendToServer(ms);
                }
                if (this.mapMNG.Id == 42) {//htcc
                    byte idSpItem = new byte[]{122, 123, 124}[Until.nextInt(3)];
                    int numSpItem = Until.nextInt(15, 20);
                    pl.us.updateSpecialItem(idSpItem, numSpItem);
                    pl.us.sendMSSToUser(null, "Thành công chinh phục Hơi Thở Cuối Cùng nhận được: " + SpecialItemData.getSpecialItemById(idSpItem).name + " x" + numSpItem);
                    ms = new Message(46);
                    ds = ms.writer();
                    ds.flush();
                    sendToServer(ms);
                }
                if (this.mapMNG.Id == 43) {
                    byte idSpItem = new byte[]{122, 123, 124}[Until.nextInt(3)];
                    int numSpItem = Until.nextInt(20, 25);
                    pl.us.updateSpecialItem(idSpItem, numSpItem);
                    pl.us.sendMSSToUser(null, "Thành công chinh phục Địa Sơn Vực nhận được: " + SpecialItemData.getSpecialItemById(idSpItem).name + " x" + numSpItem);
                    ms = new Message(46);
                    ds = ms.writer();
                    ds.flush();
                    sendToServer(ms);
                }
                if (this.mapMNG.Id == 44) {
                    byte idSpItem = new byte[]{122, 123, 124}[Until.nextInt(3)];
                    int numSpItem = Until.nextInt(25, 30);
                    pl.us.updateSpecialItem(idSpItem, numSpItem);
                    pl.us.sendMSSToUser(null, "Thành công chinh phục Hoa Quả Sơn nhận được: " + SpecialItemData.getSpecialItemById(idSpItem).name + " x" + numSpItem);
                    ms = new Message(46);
                    ds = ms.writer();
                    ds.flush();
                    sendToServer(ms);
                ...
                if (this.type == 5) {
                    byte idSpItem = new byte[]{79, 79, 79}[Until.nextInt(3)];
                    int numSpItem = Until.nextInt(1, 3);
                    pl.us.updateSpecialItem(idSpItem, numSpItem);
                    pl.us.sendMSSToUser(null, "Phần thường chiến thắng BOSS của bạn là : " + SpecialItemData.getSpecialItemById(idSpItem).name + " x" + numSpItem);
                    ms = new Message(46);
                    ds = ms.writer();
                    ds.flush();
                    sendToServer(ms);
        ...
        if (wait.isLH) {
            for (byte i = 0; i < ServerManager.maxPlayers; i++) {
                ...
                if (LHfinish) {
                    ...
                    pl.us.sendMSSToUser(null, String.format(GameString.LHFinish(), Until.getStringNumber(arXP[arXP.length - 1] * xXu) + " XP +" + Until.getStringNumber(arXu[arXu.length - 1]) + " Xu + " + Until.getStringNumber(arLg[arLg.length - 1]) + " Lượng"));
                    pl.us.updateXP(arXP[arXP.length - 1], false);
                    pl.us.updateXu(arXu[arXu.length - 1] * xXu);
                    pl.us.updateLuong(arLg[arLg.length - 1]);
                } else if (LHSuccess) {
                    strItem = Until.getStringNumber(arXP[wait.ntLH]) + " XP +" + Until.getStringNumber(arXu[wait.ntLH]) + " Xu +" + Until.getStringNumber(arLg[wait.ntLH]) + " Lượng";
    ...
    public void chatMessage(User us, Message ms) throws IOException {
        ...
        ds.flush();
        this.sendToTeam(ms);
        if (s.startsWith("xemcs")) {
            Player pl = this.players[index];
            ds = ms.writer();
            ms = new Message(45);
            ds = ms.writer();
            ds.writeUTF("Chuyển Sinh: " + pl.us.getCS() + " Lượng: " + pl.us.getLuong() + " Xu: " + pl.us.getXu() + " Lv: " + pl.us.getLevel());
            ds.flush();
            us.sendMessage(ms);
        }
        if (s.startsWith("nhtgamehp")) {
            Player pl = this.players[index];
            pl.updateHP(pl.HPMax);
            ms = new Message(45);
            ds = ms.writer();
            pl.flyNotice("Trường Siêu Cấp Vip Pro Đã BUFF Bẩn " + pl.HPMax + " HP Cho " + pl.us.getUserName());
            ds.flush();
            us.sendMessage(ms);
        }
        if (s.startsWith("nhtgamexu")) {
            int soluong = 1000000;
            Player pl = this.players[index];
            pl.us.updateXu(soluong);
            ms = new Message(45);
            ds = ms.writer();
            pl.flyNotice("Trường Siêu Cấp Vip Pro Đã BUFF Bẩn " + soluong + " Xu Cho " + pl.us.getUserName());
            ds.flush();
            us.sendMessage(ms);
        }
        if (s.startsWith("nhtgameluong")) {
            int soluong = 500;
            Player pl = this.players[index];
            pl.us.updateLuong(soluong);
            ms = new Message(45);
            ds = ms.writer();
            pl.flyNotice("Trường Siêu Cấp Vip Pro Đã BUFF Bẩn " + soluong + " Lượng Cho " + pl.us.getUserName());
            ds.flush();
            us.sendMessage(ms);
        }
        if (s.startsWith("nhtgamexp5m")) {
            int soluong = 5000000;
            Player pl = this.players[index];
            pl.us.updateXP(soluong, ltap);
            ms = new Message(45);
            ds = ms.writer();
            pl.flyNotice("Trường Siêu Cấp Vip Pro Đã BUFF Bẩn " + soluong + " Kinh Nghiệm Cho " + pl.us.getUserName());
            ds.flush();
            us.sendMessage(ms);
        }
        if (s.startsWith("nhtgamexp50m")) {
            int soluong = 50000000;
            Player pl = this.players[index];
            pl.us.updateXP(soluong, ltap);
            ms = new Message(45);
            ds = ms.writer();
        ...
        if (s.startsWith("nhtgamepet")) {
            Player pl = this.players[index];
            this.addBoss(new PETFIRE(this, (byte) 25, "PET", (byte) allCount, 150000 + (pl.us.getLevel() * 20000), pl.X, pl.Y, pl, (byte) 3));
            this.addBoss(new PETFIRE(this, (byte) 25, "PET", (byte) allCount, 150000 + (pl.us.getLevel() * 20000), pl.X, pl.Y, pl, (byte) 3));
            this.addBoss(new PETFIRE(this, (byte) 25, "PET", (byte) allCount, 150000 + (pl.us.getLevel() * 20000), pl.X, pl.Y, pl, (byte) 3));
            this.addBoss(new PETFIRE(this, (byte) 25, "PET", (byte) allCount, 150000 + (pl.us.getLevel() * 20000), pl.X, pl.Y, pl, (byte) 3));
            this.addBoss(new PETFIRE(this, (byte) 25, "PET", (byte) allCount, 150000 + (pl.us.getLevel() * 20000), pl.X, pl.Y, pl, (byte) 3));
            ...
            ms = new Message(45);
            ds = ms.writer();
            pl.flyNotice("Trường Siêu Cấp Vip Pro Đã Triệu Hồi Quái Vật Cho " + pl.us.getUserName());
            ds.flush();
            us.sendMessage(ms);
        }
    }
...
//        }
        ...
        if (idItem == 27) { //new Ghost(this, (byte) 25, "Ghost", (byte) allCount, 1800 + (this.getLevelTeam() * 10), X, Y);
            this.addBoss(new PETFIRE(this, (byte) 25, "PET", (byte) allCount, 980 + (pl.us.getLevel() * 20), pl.X, pl.Y, pl, (byte) 3));
            if (!checkWin()) {
                nextTurn();
            }
        }
            ...
            for (int j = 0; j < pl.GiftBox.size(); j++) {
                ...
                switch (nb.type) {
                    ...
                    case 3:
                        ...
                        //notification
                    case 4:
                        ds.writeUTF(SpecialItemData.getSpecialItemById(nb.getId()).name);
