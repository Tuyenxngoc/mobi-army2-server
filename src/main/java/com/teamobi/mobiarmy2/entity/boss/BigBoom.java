...
public class BigBoom extends Boss {
    public BigBoom(FightManager fightManager, byte index, short x, short y, short maxHp) {
    ...
    public void turnAction() {
        ...
        if (Utils.nextInt(100) < 45) {
            ...
            fightManager.sendShootData(index, (byte) 34, angle, force, (byte) 0, (byte) 1);
        } else {
            moveToTarget(player);
            int distance = calculateDistance(player.getX(), player.getY());
            if (distance < 35) {
                fightManager.sendShootData(index, (byte) 31, (short) 0, (byte) 0, (byte) 0, (byte) 1);
                return;
            }
            skipTurn();
        }
