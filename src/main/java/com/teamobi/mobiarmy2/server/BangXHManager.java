...
public class BangXHManager {
    ...
    protected static final ArrayList<ClanManager.ClanEntry> topTeam = new ArrayList<>();
    ...
    public static void refreshTopTeam() {
        try {
            ClanManager.ClanEntry clan = null;
            ResultSet red = HikariCPManager.getInstance().getConnection().createStatement()
                    .executeQuery("SELECT * FROM `clan` ORDER BY `xp` DESC, `cup` DESC LIMIT 100;");
            while (red.next()) {
                clan = new ClanManager.ClanEntry();
    ...
    public static ClanManager.ClanEntry[] getTopTeam(int page) {
        ArrayList<ClanManager.ClanEntry> bxh = topTeam;
        int index = page * 10, lent = 0;
        if (index >= bxh.size()) {
            index = 0;
        }
        ...
        ClanManager.ClanEntry[] bxhA = new ClanManager.ClanEntry[lent];
        for (int i = 0; i < lent; i++) {
            if (bxh.size() <= index + i && index + i >= 0) {
                continue;
            }
