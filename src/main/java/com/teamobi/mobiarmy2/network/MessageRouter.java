    ...
    public void onMessage(Message ms) {
        try {
            switch (ms.getCommand()) {
                ...
                case Cmd.CHANGE_ROOM_NAME -> userMessageHandler.sendRoomName();
                ...
                case Cmd.TRAINING_MAP -> userMessageHandler.enterTrainingMap();

                case Cmd.SIGN_OUT -> userMessageHandler.handleLogout(ms);
                ...
                case Cmd.CHAT_TO -> userMessageHandler.handleSendMessage(ms);

                case Cmd.ROOM_LIST -> userMessageHandler.handleSendRoomList();

                case Cmd.BOARD_LIST -> userMessageHandler.handleEnteringRoom(ms);

                case Cmd.JOIN_BOARD -> userMessageHandler.handleJoinBoard(ms);

                case Cmd.CHAT_TO_BOARD -> userMessageHandler.handleChatMessage(ms);

                case Cmd.KICK -> userMessageHandler.handleKickPlayer(ms);

                case Cmd.LEAVE_BOARD -> userMessageHandler.handleLeaveBoard(ms);

                case Cmd.READY -> userMessageHandler.setReady(ms);

                case Cmd.IMBUE -> userMessageHandler.imbueGem(ms);

                case Cmd.SET_PASS -> userMessageHandler.handleSetPasswordFightWait(ms);

                case Cmd.SET_MONEY -> userMessageHandler.handleSetMoneyFightWait(ms);

                case Cmd.START_ARMY -> userMessageHandler.handleStartGame();

                case Cmd.MOVE_ARMY -> userMessageHandler.movePlayer(ms);

                case Cmd.FIRE_ARMY -> userMessageHandler.shoot(ms);

                case Cmd.SHOOT_RESULT -> userMessageHandler.processShootingResult(ms);

                case Cmd.USE_ITEM -> userMessageHandler.handleUseItem(ms);

                case Cmd.JOIN_ANY_BOARD -> userMessageHandler.handleJoinAnyBoard(ms);
                ...
                case Cmd.PLAYER_DETAIL -> userMessageHandler.handleGetFlayerDetail(ms);

                case Cmd.SEARCH -> friendMessageHandler.handleFindPlayer(ms);

                case Cmd.PING -> userMessageHandler.ping(ms);

                case Cmd.SKIP -> userMessageHandler.skipTurn();

                case Cmd.UPDATE_XY -> userMessageHandler.updateCoordinates(ms);

                case Cmd.SET_BOARD_NAME -> userMessageHandler.handleSetFightWaitName(ms);

                case Cmd.SET_MAX_PLAYER -> userMessageHandler.handleSetMaxPlayerFightWait(ms);

                case Cmd.SET_PROVIDER -> authMessageHandler.getProvider(ms);

                case Cmd.CHOOSE_ITEM -> userMessageHandler.handleChoseItemFight(ms);

                case Cmd.CHOOSE_GUN -> userMessageHandler.handleChoseCharacter(ms);

                case Cmd.CHANGE_TEAM -> userMessageHandler.handleChangeTeam(ms);

                case Cmd.BUY_ITEM -> shopMessageHandler.handlePurchaseItem(ms);

                case Cmd.BUY_GUN -> shopMessageHandler.handleBuyCharacter(ms);

                case Cmd.MAP_SELECT -> userMessageHandler.handleSelectMap(ms);

                case Cmd.LOAD_CARD -> userMessageHandler.handleCardRecharge(ms);

                case Cmd.FIND_PLAYER -> userMessageHandler.handleFindPlayerWait(ms);

                case Cmd.CHECK_CROSS -> userMessageHandler.clearBullet(ms);

                case Cmd.CHANGE_PASS -> userMessageHandler.handleChangePassword(ms);

                case Cmd.TRAINING -> userMessageHandler.startTraining(ms);

                case Cmd.TRAININGSHOOT -> userMessageHandler.trainShooting(ms);
