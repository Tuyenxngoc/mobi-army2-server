...
public class LoginService extends BaseService {
    ...
    public void sendCharacterData(ServerConfig config) {
        try {
            List<com.teamobi.mobiarmy2.entity.Character> characterEntries = CharacterManager.CHARACTERS;
            int characterCount = characterEntries.size();
            Message ms = new Message(Cmd.SKIP_2);
            DataOutputStream ds = ms.writer();
            ds.writeByte(characterCount);
            for (com.teamobi.mobiarmy2.entity.Character character : characterEntries) {
