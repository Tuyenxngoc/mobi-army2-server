...
`MobiArmy2` → `BeanRegistry` manually instantiates every singleton and wires dependencies → `ApplicationContext` acts as a service locator. There is no DI framework; all wiring is explicit in `BeanRegistry.java`.
...
`MessageRouter` dispatches on `Message.command` (byte) to one of 16 `BaseMessageHandler` subclasses (Auth, Fight, Shop, Clan, Leaderboard, …). Helper methods `us()`, `fw()`, `fm()` on the base handler resolve the current User, FightWait, and FightManager from the session.

`Message` wraps a command byte + `DataInputStream`/`DataOutputStream` — all game communication is compact binary, not JSON.
...
**Combat:** `FightManager` runs a turn-based loop (max 8 players). Each turn: receive aim/fire command → `BulletFactory` creates one of 50+ `Bullet` subclasses → physics + collision resolved server-side → results broadcast to all players. Wind, terrain collision, and special-item effects (shields, nukes, portals) are all computed here. Boss AI is encoded in 19 `Boss` subclasses that override `getNextAction()`.
...
| `TrainingManager` | Single-player training mode |
